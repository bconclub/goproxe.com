...
export async function GET(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return Response.json({ agent_id: d.agent_id, conversation_id: d.conversation_id, status: d.status, has_audio: !!d.has_audio,
      metadata: { start_time_unix_secs: d.metadata?.start_time_unix_secs, call_duration_secs: d.metadata?.call_duration_secs, phone_call: { external_number: d.metadata?.phone_call?.external_number || null }, error: { message: d.metadata?.error?.message || null } },
